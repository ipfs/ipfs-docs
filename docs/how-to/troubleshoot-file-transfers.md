...
To start, make sure that ipfs is running on both machines. To verify, run `ipfs id` on each machine and check if the `Addresses` field has anything in it. If
it says `null`, then your node is not online and you will need to run `ipfs daemon`.
...
peer ID in the output. If the two nodes _are_ connected, and the `ipfs get`
...
node B is able to determine that node A has the data. Run `ipfs dht findprovs <hash>`. We expect to see the peer ID of node A printed out. If this command
...
while node A does not have a daemon running. If this happens, you can run `ipfs dht provide <hash>` on node A to announce to the network that you have that
...
192.168._._ one) and another address. If this third address matches your
...
doc](./experimental-features.md#circuit-relay). If that _still_ doesn't work,
then you should either join IRC and ask for help there, or file an issue on
github.

If this manual step _did_ work, then you likely have an issue with NAT
