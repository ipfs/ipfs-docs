...
[Iotex](https://www.iotex.io/) is a harware company also providing a secure platform to host the data generated by the user, with a particular attention to privacy. Owners of their [Ucam security camera](https://ucam.iotex.io/) can chose to store it locally, on a cloud, or on an IPFS-backed storage. Iotex provides a blockchain managing identities ensuring only the owner of the data can access them, not even Iotex.

### Exchange Internet-of-Things data

[IOTA](https://www.iota.org/) is a foundation maintaining the Tangle, a blockchain-like network with zero fee. Their vision is to automate exchange of data between sensors, machines and other devices, for free or sold by the provider. IOTA [announced](https://docs.iota.org/docs/blueprints/0.1/tangle-data-storage/overview) that data can now be hosted on IPFS as shown on this [demo](https://ipfs.iota.org/). 
