...
arg `arg` [string]: A peer to add to the bootstrap list (in the format `<multiaddr>/<peerID>`) Required: no.
...
`curl -X POST "http://localhost:5001/api/v0/files/stat?arg=<path>&format=<hash> Size: <size> CumulativeSize: <cumulsize> ChildBlocks: <childs> Type: <type>&hash=<value>&size=<value>&with-local=<value>"`
