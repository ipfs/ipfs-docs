...
::: tip Generated on 2026-09-15 01:13:21, from kubo 0.43.1
This document was autogenerated from CLI help text in [kubo 0.43.1](https://github.com/ipfs/kubo/releases/tag/v0.43.1)
...
DESCRIPTION
  ...
  'ipfs config show' does not work here because it has no private key.
  Profiles given with --profile are applied on top of the file.
  ...
  Two experimental, opt-in profiles change this: 'flatfs-pebbleds' keeps
  flatfs for blocks and uses pebble instead of leveldb, and 'pebbleds' puts
  everything in pebble. Details and trade-offs:
...
SYNOPSIS
  ipfs ls [--headers | -v] [--resolve-type=false] [--size=false] [--stream | -s]
          [--long | -l] [--human | -H] [--sort-size | -S] [--] <ipfs-path>...
...
OPTIONS
  ...
  -l, --long       bool - Use a long listing format, showing file mode and
                          modification time.
  -H, --human      bool - Print sizes in human readable format (e.g., 1.2 kB,
                          234 MB, 2.0 GB).
  -S, --sort-size  bool - Sort entries by size, largest first (directories
                          treated as size 0).
