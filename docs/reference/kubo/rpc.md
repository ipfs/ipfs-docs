...
::: tip Generated on 2024-01-22, from kubo v0.26.0
This document was autogenerated from [v0.26.0](https://github.com/ipfs/kubo/releases/tag/v0.26.0).
...
- `collectors` [array]: The list of collectors to use for collecting diagnostic data. Default: [goroutines-stack goroutines-pprof version heap allocs bin cpu mutex block]. Default: `[goroutines-stack goroutines-pprof version heap allocs bin cpu mutex block]`. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/diag/profile?output=<value>&collectors=[goroutines-stack goroutines-pprof version heap allocs bin cpu mutex block]&profile-time=30s&mutex-profile-fraction=4&block-profile-rate=1ms"`
...
- `names` [bool]: Enable displaying pin names (slower). Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/pin/ls?arg=<ipfs-path>&type=all&quiet=<value>&stream=<value>&names=<value>"`
...
`curl -X POST "http://127.0.0.1:5001/api/v0/dht/get?arg=<key>"`
...
`curl -X POST -F file=@myfile "http://127.0.0.1:5001/api/v0/dht/put?arg=<key>&allow-offline=<value>"`
