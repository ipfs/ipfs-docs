...
::: tip Generated on 2026-09-15, from kubo v0.43.1
This document was autogenerated from [v0.43.1](https://github.com/ipfs/kubo/releases/tag/v0.43.1).
...
`curl -X POST "http://127.0.0.1:5001/api/v0/get?arg=<ipfs-path>&output=<value>&archive=<value>&compress=<value>&compression-level=<value>&progress=<value>"`
...
- `human` [bool]: Print sizes in human readable format (e.g., 1.2 kB, 234 MB, 2.0 GB). Required: no.
- `sort-size` [bool]: Sort entries by size, largest first (directories treated as size 0). Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/ls?arg=<ipfs-path>&headers=<value>&resolve-type=true&size=true&stream=<value>&long=<value>&human=<value>&sort-size=<value>"`
...
- `fast-provide-root` [bool]: Immediately provide root CID to DHT after pinning.FastProvideRoot. Required: no.
- `fast-provide-dag` [bool]: Walk and provide the full DAG according to Provide.Strategy after pinning.FastProvideDAG. Required: no.
- `fast-provide-wait` [bool]: Block until the immediate provide completes.FastProvideWait. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/pin/add?arg=<ipfs-path>&recursive=true&name=<value>&progress=<value>&fast-provide-root=<value>&fast-provide-dag=<value>&fast-provide-wait=<value>"`
...
- `fast-provide-root` [bool]: Immediately provide new root CID to DHT after update.FastProvideRoot. Required: no.
- `fast-provide-dag` [bool]: Walk and provide the full DAG according to Provide.Strategy after update.FastProvideDAG. Required: no.
- `fast-provide-wait` [bool]: Block until the immediate provide completes.FastProvideWait. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/pin/update?arg=<from-path>&arg=<to-path>&unpin=true&fast-provide-root=<value>&fast-provide-dag=<value>&fast-provide-wait=<value>"`
...
Announce CIDs to the routing system on demand.

### Arguments

- `arg` [string]: The CID(s) to announce. Required: **yes**.
- `recursive` [bool]: Recursively announce the entire DAG. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/provide/once?arg=<cid>&recursive=<value>"`
...
Show statistics about the provide system
...
- `allow-non-unixfs` [bool]: Skip UnixFS validation, allowing links on non-directory nodes. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/object/patch/add-link?arg=<root>&arg=<name>&arg=<ref>&create=<value>&allow-non-unixfs=<value>"`
...
- `allow-non-unixfs` [bool]: Skip UnixFS validation, allowing link removal on non-directory nodes. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/object/patch/rm-link?arg=<root>&arg=<name>&allow-non-unixfs=<value>"`
...
Deprecated, use &#39;ipfs provide once&#39; instead.
...
Trigger a reprovide cycle (legacy provider only).
