...
- You can enable automatic garbage collection via [--enable-gc](../reference/kubo/cli.md#ipfs-daemon) and adjust using [default maximum disk storage](https://github.com/ipfs/kubo/blob/v0.43.1/docs/config.md#datastorestoragemax) for data retrieved from other peers.
...
1. Download the Linux binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.43.1/kubo_v0.43.1_linux-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.43.1_linux-amd64.tar.gz
...
1. Download the Windows binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```powershell
   wget https://dist.ipfs.tech/kubo/v0.43.1/kubo_v0.43.1_windows-amd64.zip -Outfile kubo_v0.43.1.zip
   ```

1. Unzip the file to a sensible location, such as `~\Apps\kubo_v0.43.1`.

   ```powershell
   Expand-Archive -Path kubo_v0.43.1.zip -DestinationPath ~\Apps\kubo_v0.43.1
   ```

1. Move into the `kubo_v0.43.1` folder

   ```powershell
   cd ~\Apps\kubo_v0.43.1\kubo
   ```
...
> The `brew` installation method supports both Intel and Apple Silicon hardware. If you prefer a manual installation, `darwin-amd64` (Intel) and `darwin-arm64` (Apple Silicon) artifacts available [here](https://dist.ipfs.tech/#kubo).
...
1. Download the FreeBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.43.1/kubo_v0.43.1_freebsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.43.1_freebsd-amd64.tar.gz
...
1. Download the OpenBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.43.1/kubo_v0.43.1_openbsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.43.1_openbsd-amd64.tar.gz
...
For the current instructions on how to manually download, compile and build Kubo from source, see the [Build from Source](https://github.com/ipfs/kubo/blob/v0.43.1/README.md#build-from-source) section in the Kubo repository.
