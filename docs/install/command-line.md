...
- You can enable automatic garbage collection via [--enable-gc](../reference/kubo/cli.md#ipfs-daemon) and adjust using [default maximum disk storage](https://github.com/ipfs/kubo/blob/v0.19.2/docs/config.md#datastorestoragemax) for data retrieved from other peers.
...
1. Download the Windows binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```powershell
   wget https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_windows-amd64.zip -Outfile kubo_v0.19.2.zip
   ```

1. Unzip the file to a sensible location, such as `~\Apps\kubo_v0.19.2`.

   ```powershell
   Expand-Archive -Path kubo_v0.19.2.zip -DestinationPath ~\Apps\kubo_v0.19.2
   ```

1. Move into the `kubo_v0.19.2` folder

   ```powershell
   cd ~\Apps\kubo_v0.19.2\kubo
   ```
...
1. Download the appropriate macOS binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo) based on your hardware. For example:
   ...
   ```bash
   curl -O https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_darwin-amd64.tar.gz
   ```
   ...
   - _If you are using hardware with Apple Silicon, download the `darwin-arm64` binary._ For example, to download the IPFS binary for `Kubo v0.19.2` for an Apple-based system, run the following command:

     ```bash
     curl -O https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_darwin-arm64.tar.gz
     ```

   - _If you are using hardware with an Intel Processor, download the `darwin-amd64` binary._ For example, to download the IPFS binary for `Kubo v0.19.2` for an Intel-based system, run the following command:

     ```bash
     curl -O https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_darwin-amd64.tar.gz
     ```

1. Unzip the file. For example, to unzip `Kubo v0.19.2` for an Intel-based system:

   ```bash
   tar -xvzf kubo_v0.19.2_darwin-amd64.tar.gz
   ```
...
1. Download the Linux binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_linux-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.19.2_linux-amd64.tar.gz
...
1. Download the FreeBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_freebsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.19.2_freebsd-amd64.tar.gz
...
1. Download the OpenBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/v0.19.2/kubo_v0.19.2_openbsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_v0.19.2_openbsd-amd64.tar.gz
...
For the current instructions on how to manually download, compile and build Kubo from source, see the [Build from Source](https://github.com/ipfs/kubo/blob/v0.19.2/README.md#build-from-source) section in the Kubo repository.
