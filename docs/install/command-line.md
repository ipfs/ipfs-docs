...
- You can enable automatic garbage collection via [--enable-gc](../reference/kubo/cli.md#ipfs-daemon) and adjust using [default maximum disk storage](https://github.com/ipfs/kubo/blob/vull/docs/config.md#datastorestoragemax) for data retrieved from other peers.
...
If you are running Kubo on a resource-constrained system (such as a Raspberry Pi), you should complete the following steps:
  ...
  1. Install OpenSSL and rebuild Kubo manually with `make build GOTAGS=openssl`. See the [download and compile](https://github.com/ipfs/kubo/blob/vull/README.md#download-and-compile-ipfs) section for more information on compiling Kubo.
...
1. Download the Windows binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```powershell
   wget https://dist.ipfs.tech/kubo/vull/kubo_vull_windows-amd64.zip -Outfile kubo_vull.zip
   ```

1. Unzip the file to a sensible location, such as `~\Apps\kubo_vull`.

   ```powershell
   Expand-Archive -Path kubo_vull.zip -DestinationPath ~\Apps\kubo_vull
   ```

1. Move into the `kubo_vull` folder

   ```powershell
   cd ~\Apps\kubo_vull\kubo
   ```
...
1. Download the appropriate macOS binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo) based on your hardware. For example:
   ...
   ```bash
   curl -O https://dist.ipfs.tech/kubo/vull/kubo_vull_darwin-amd64.tar.gz
   ```
   ...
   - _If you are using hardware with Apple Silicon, download the `darwin-arm64` binary._ For example, to download the IPFS binary for `Kubo vull` for an Apple-based system, run the following command:

     ```bash
     curl -O https://dist.ipfs.tech/kubo/vull/kubo_vull_darwin-arm64.tar.gz
     ```

   - _If you are using hardware with an Intel Processor, download the `darwin-amd64` binary._ For example, to download the IPFS binary for `Kubo vull` for an Intel-based system, run the following command:

     ```bash
     curl -O https://dist.ipfs.tech/kubo/vull/kubo_vull_darwin-amd64.tar.gz
     ```

1. Unzip the file. For example, to unzip `Kubo vull` for an Intel-based system:

   ```bash
   tar -xvzf kubo_vull_darwin-amd64.tar.gz
   ```
...
1. Download the Linux binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/vull/kubo_vull_linux-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_vull_linux-amd64.tar.gz
...
1. Download the FreeBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/vull/kubo_vull_freebsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_vull_freebsd-amd64.tar.gz
...
1. Download the OpenBSD binary from [`dist.ipfs.tech`](https://dist.ipfs.tech/#kubo).
   ...
   ```bash
   wget https://dist.ipfs.tech/kubo/vull/kubo_vull_openbsd-amd64.tar.gz
   ```

1. Unzip the file:

   ```bash
   tar -xvzf kubo_vull_openbsd-amd64.tar.gz
...
For the current instructions on how to manually download, compile and build Kubo from source, see the [Build from Source](https://github.com/ipfs/kubo/blob/vull/README.md#build-from-source) section in the Kubo repository.
