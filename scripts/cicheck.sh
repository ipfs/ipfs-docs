...
git remote add fdocs https://$${GH_TOKEN}@github.com/ipfs/ipfs-docs.git > /dev/null 2>&1
git fetch fdocs
...
OLDCOMMENTSJSON=$(curl -H "Authorization: token $GH_TOKEN"  -X GET https://api.github.com/repos/ipfs/ipfs-docs/issues/$PR_NUMBER/comments)
OLDCOMMENTS=$(echo $OLDCOMMENTSJSON | jq ".[] | select(.user.id==$GH_USER_ID) | .id" --jsonargs)

for i in $OLDCOMMENTS; do curl -i -H "Authorization: token $GH_TOKEN" -X DELETE https://api.github.com/repos/ipfs/ipfs-docs/issues/comments/$i; done
...
curl -i -H "Authorization: token $GH_TOKEN" \
    ...
    https://api.github.com/repos/ipfs/ipfs-docs/issues/$PR_NUMBER/comments
